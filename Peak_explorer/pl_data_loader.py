    ...
    def load_h5_data(self, mode, h5_path):
        ...
        with h5py.File(h5_path, "r") as f:
            if mode == "pl_raw":
                ...
                data_matrix = f["/raw_optical_measurements/raw_pl_measurements/raw_pl_data"][()]
            ...
            if mode == "pl_binned":
                ...
                data_matrix = f["/binned_optical_measurements/binned_pl_measurements_bg"][()].T
                timestamps, wavelengths = get_axes_from_extent(extent, data_matrix)
            if mode == "giwaxs":
                timestamps = f["/beamline_logging/Time"][()]
                data_matrix = f["/diffractogram/i_values"][()]
                wavelengths = f["/diffractogram/q_values"][()][0] / 10

        # delete last entry if nan
