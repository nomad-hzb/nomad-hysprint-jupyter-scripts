...
def is_running_in_jupyter():
    """Helper function to check if code is running in Jupyter"""
    try:
...
def jv_plot_by_cell_3x2(df, sample, path):
    ...
    for i, (cell, group) in enumerate(grouped):
        ...
        # Extract 'Voltage (V)' and 'Current Density(mA/cm²)' for each direction and illumination
        for direction in group['direction'].unique():
            for ilum in group['ilum'].unique():
                filtered_data = group[(group["direction"] == direction) & (group["ilum"] == ilum)]
                
                if filtered_data.empty:
                    continue
                    
                plotted = filtered_data.copy().drop(
                ...
                if "Voltage (V)" in plotted.columns and "Current Density(mA/cm2)" in plotted.columns:
                    fig.add_trace(
                        go.Scatter(
                            x=plotted["Voltage (V)"], 
                            y=plotted["Current Density(mA/cm2)"],
                            mode="lines+markers",
                            name=label,
                            legendgroup=label,
                            showlegend=(i == 0),  # Only show legend for the first cell
                            hovertemplate='Voltage: %{x:.3f} V<br>Current Density: %{y:.3f} mA/cm²<br>Cell: ' + cell + '<br>' + label
                        ),
                        row=row, col=col
                    )
